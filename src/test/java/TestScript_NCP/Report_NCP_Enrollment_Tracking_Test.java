...
import ObjectRepository_NCP.Report_NCP_Enrollment_Tracking;

public class Report_NCP_Enrollment_Tracking_Test extends BaseClass
{
	WebDriverCommonLib wb = new WebDriverCommonLib();
	public String reportName = "Enrollment Tracking Report";

	@Test(dataProvider = "excelData")
	public void testEnrollmentTrackingReport(String ProgramName, String Division, String Projects)
			throws Throwable {
		...
		Thread.sleep(1000);
//		wb.selectSingleValueByIndex(Report_NCP_Enrollment_Tracking.selectProgram(driver), 1);
		test.log(LogStatus.PASS, Projects + " Project Selected");
		Thread.sleep(1000);
		
		try {
			wb.selectSingleValue(Report_NCP_Enrollment_Tracking.selectContractor(driver), "All Contractors");
		...
		Report_NCP_Enrollment_Tracking.btnBuildReport(driver).click();
		...
		try {
			...
			Thread.sleep(2000);
			System.out.println(Report_NCP_Enrollment_Tracking.reportHeader(driver).getText());
			Assert.assertEquals(Report_NCP_Enrollment_Tracking.reportHeader(driver).getText(),
					"Enrollment Tracker Report");
