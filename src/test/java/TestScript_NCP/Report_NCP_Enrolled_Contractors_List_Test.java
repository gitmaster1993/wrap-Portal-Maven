...
import ObjectRepository_NCP.Report_NCP_Enrolled_Contractors_List;

public class Report_NCP_Enrolled_Contractors_List_Test extends BaseClass {

	WebDriverCommonLib wb = new WebDriverCommonLib();
	String reportName = "Enrolled Contractors";

	@Test(dataProvider = "excelData")
	public void testEnrolledContractorsNCP(String ProgramName, String Division, String Projects)
		...
		try {
			...
			wb.selectSingleValue(Report_NCP_Enrolled_Contractors_List.filterSelectDivisionDropdown(driver), Division);
			test.log(LogStatus.PASS, Division + " Selected Successfully");
			Report_NCP_Enrolled_Contractors_List.filterSelectProjectFormOption(driver, Projects).click();
			test.log(LogStatus.PASS, Projects + " Project Selected Successfully");

			Report_NCP_Enrolled_Contractors_List.btnBuildReport(driver).click();
		...
		try {
			System.out.println(Report_NCP_Enrolled_Contractors_List.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_NCP_Enrolled_Contractors_List.reportHeaderReportName(driver).getText(),
		...
		// filter testing
		Report_NCP_Enrolled_Contractors_List.filterWindowButton(driver).click();
		Thread.sleep(3000);
		Report_NCP_Enrolled_Contractors_List.filterSelectProjectFormOption(driver, "Test Project 10").click();
		test.log(LogStatus.PASS, " First Project Selected Successfully");
		int contPkgSize = 11;
		// add second project
		Report_NCP_Enrolled_Contractors_List.filterSelectProjectFormOption(driver, "Test 132 OFF").click();
		...
		Report_NCP_Enrolled_Contractors_List.pdfLink(driver).click();
		...
		// Excel file
		Report_NCP_Enrolled_Contractors_List.excelLink(driver).click();
		...
		Thread.sleep(3000);
		Report_NCP_Enrolled_Contractors_List.addFilterBtnOutside(driver).click();
		test.log(LogStatus.PASS, "Clicked on Add Filter Button From Filter");
		wb.getWindowID3();
		driver.switchTo().window(wb.map.get("subchildID"));
		driver.manage().window().maximize();
		wb.waitForElemnet(Report_NCP_Enrolled_Contractors_List.selectAnFilterDropdown(driver));
		wb.selectSingleValueByIndex(Report_NCP_Enrolled_Contractors_List.selectAnFilterDropdown(driver), 1);
		Thread.sleep(3000);
		test.log(LogStatus.PASS, " Filter Selected Successfully From Dropdown");
		wb.selectSingleValueByIndex(Report_NCP_Enrolled_Contractors_List.selectAFilterCondition(driver), 1);
		Thread.sleep(4000);
		test.log(LogStatus.PASS, "Filter Condition Selected Successfully ");
		Report_NCP_Enrolled_Contractors_List.addFilterParameter1(driver).click();
		Thread.sleep(1000);
		wb.selectSingleValueFromList(Report_NCP_Enrolled_Contractors_List.selectParameter1(driver), "Enrolled");
		test.log(LogStatus.PASS, "Parameter Selected Successfully ");
		Report_NCP_Enrolled_Contractors_List.addFilterBtnInside(driver).click();
