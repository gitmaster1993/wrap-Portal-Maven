...
import ObjectRepository_NCP.Report_NCP_Contract_Status_Report;

public class Report_NCP_Contract_Status_Report_Test extends BaseClass {
	WebDriverCommonLib wb = new WebDriverCommonLib();
	String reportName = "Contract Status Report";

	@Test(dataProvider = "excelData")
	public void testContractStatusReport(String ProgramName, String Division, String Projects)
			throws Throwable {
		...
		try {
			...
			wb.selectSingleValue(Report_NCP_Contract_Status_Report.filterSelectDivisionDropdown(driver), Division);
			test.log(LogStatus.PASS, Division + " Divisions Selected Successfully");
			Report_NCP_Contract_Status_Report.filterSelectProjectFormOption(driver, Projects).click();
			test.log(LogStatus.PASS, Projects + " Project Selected Successfully");

			wb.selectSingleValue(Report_NCP_Contract_Status_Report.filterSelectContractorFormOption(driver),
					"All Contractors");
			test.log(LogStatus.PASS, " Contractor Selected Successfully");

			Report_NCP_Contract_Status_Report.btnBuildReport(driver).click();
		...
		// String
		// reportName=Report_NCP_Contract_Status_Report.reportHeaderReportName(driver,
		// "Contractor Enrollment Summary").getText();

		try {

			SoftAssert softAssertion = new SoftAssert();
			System.out.println(Report_NCP_Contract_Status_Report.reportHeaderReportName(driver).getText());
			softAssertion.assertEquals(Report_NCP_Contract_Status_Report.reportHeaderReportName(driver).getText(),
					"Contract Status Report");
			System.out.println("Report Name is Matched");
			test.log(LogStatus.PASS, "Report Header Name Matched");
			test.log(LogStatus.PASS, "Report Generated Successfully");
			softAssertion.assertEquals(Report_NCP_Contract_Status_Report.programName(driver).getText(), ProgramName);
			test.log(LogStatus.PASS,
					"Program Name:- " + Report_NCP_Contract_Status_Report.programName(driver).getText());
			test.log(LogStatus.PASS,
					"Sponsor Name:- " + Report_NCP_Contract_Status_Report.sponsorName(driver).getText());
			test.log(LogStatus.PASS,
					"Project Name:- " + Report_NCP_Contract_Status_Report.projectName(driver).getText());

			test.log(LogStatus.PASS, "Time Stamp:- " + Report_NCP_Contract_Status_Report.timeStamp(driver).getText());
			/*
			 * test.log(LogStatus.PASS, "PDF Option is Present:- " +
			 * Report_NCP_Contract_Status_Report.exportToPDF(driver).isDisplayed());
			 */  //Removed in PDF 
			
			Thread.sleep(2000);
			test.log(LogStatus.PASS, "Excel Option is Present:- "
					+ Report_NCP_Contract_Status_Report.ExportToExcel(driver).isDisplayed());

			test.log(LogStatus.PASS,
					"Total Contractor:- " + Report_NCP_Contract_Status_Report.timeStamp(driver).getText());
			test.log(LogStatus.PASS, "================= Column Name =================");
			List<WebElement> list = Report_NCP_Contract_Status_Report.allColumnName(driver);
			// wb.printListData();
		...
		// wb.printListData(Report_NCP_Contract_Status_Report.countContractorPkgStatus(driver));
		test.log(LogStatus.PASS, "===================Below Are Contractor Package Status and Info===================");
		List<WebElement> lst = Report_NCP_Contract_Status_Report.countContractorPkgStatus(driver);
		 ...
		 * test.addScreenCapture(FileUtils1.getScreenshot(driver))); Thread.sleep(5000);
