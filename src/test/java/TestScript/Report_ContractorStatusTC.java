...
import ObjectRepository.Report_ContractorStatus;

public class Report_ContractorStatusTC extends BaseClass {

	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "Contractor Status";
//	String project1 = "120 Piedmont Student Housing";
//	String project2 = "712 West Peachtree";

	@Test(dataProvider = "excelData")
	public void testContractorStatus(String ProgramName, String Division, String Projects, String userName,
...
//		WPO_LoginActionClientWise.logInLockton(driver);
			...
			Thread.sleep(2000);
			wb.selectSingleValue(Report_ContractorStatus.selectProgramDropdown(driver), ProgramName);
			test.log(LogStatus.INFO, ProgramName + " Program Select Form Dropdown");
			Thread.sleep(5000);
			Report_ContractorStatus.selectReport(driver, selectReportName).click();
			 ...
			 * Report_ContractorStatus.filterSelectDivisionDropdown(driver), Division);
			 * wb.selectSingleValue(Report_ContractorStatus.filterSelectDivisionDropdown(
			 * driver), Division); test.log(LogStatus.PASS, Division +
			 * " Selected Successfully");
			 * 
			 * js.executeScript("arguments[0].scrollIntoView();",Report_ContractorStatus.
			 * filterSelectProjectFormOption(driver, Projects));
			 * Report_ContractorStatus.filterSelectProjectFormOption(driver,
			 * Projects).click();
			 * 
			 * test.log(LogStatus.PASS, Projects + " Project Selected Successfully");
			 */
			js.executeScript("arguments[0].scrollIntoView();",Report_ContractorStatus.btnBuildReport(driver));
			Report_ContractorStatus.btnBuildReport(driver).click();
		...
		try {
			System.out.println(Report_ContractorStatus.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_ContractorStatus.reportHeaderReportName(driver).getText(), "Contractor Status");
		...
		// filter testing
		Report_ContractorStatus.filterWindowButton(driver).click();
		Thread.sleep(3000);
		Report_ContractorStatus.filterSelectProjectFormOption(driver, ProjectNAME1).click();
		...
		Report_ContractorStatus.filterWindowButton(driver).click();
		Thread.sleep(3000);
		Report_ContractorStatus.filterSelectProjectFormOption(driver, ProjectNAME2).click();
...
//			testCount(contPkgSize);
		...
		Report_ContractorStatus.pdfLink(driver).click();
		...
		// Excel file
		Report_ContractorStatus.excelLink(driver).click();
		...
		Thread.sleep(3000);
		Report_ContractorStatus.addFilterBtnOutside(driver).click();
		test.log(LogStatus.PASS, "Clicked on Add Filter Button From Filter");
		wb.getWindowID3();
		driver.switchTo().window(wb.map.get("subchildID"));
		driver.manage().window().maximize();
		wb.waitForElemnet(Report_ContractorStatus.selectAnFilterDropdown(driver));
		wb.selectSingleValueByIndex(Report_ContractorStatus.selectAnFilterDropdown(driver), 1);
		Thread.sleep(3000);
		test.log(LogStatus.PASS, " Filter Selected Successfully From Dropdown");
		wb.selectSingleValueByIndex(Report_ContractorStatus.selectAFilterCondition(driver), 1);
		Thread.sleep(3000);
		test.log(LogStatus.PASS, "Filter Condition Selected Successfully ");
		Report_ContractorStatus.addFilterParameter1(driver).click();
		Thread.sleep(1000);
		wb.selectSingleValueFromList(Report_ContractorStatus.selectParameter1(driver), "Enrolled");
		test.log(LogStatus.PASS, "Parameter Selected Successfully ");
		Report_ContractorStatus.addFilterBtnInside(driver).click();
	...
	public void testCount(int contPkgSize) {
		...
		List<WebElement> countOfContractorPkg = Report_ContractorStatus.countContractorPkg(driver);
