...
public class Report_PayrollComparisonTC extends BaseClass {
	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "Payroll Comparison";

//	String project1 = "120 Piedmont Student Housing";
//	String project2 = "712 West Peachtree";

	@Test(dataProvider = "excelData")
	public void testPayrollComparison(String ProgramName, String Division, String Projects, String userName,
...
//			WPO_LoginActionClientWise.logInLockton(driver);
			...
			//Raja commneted as filters not available fot Division
			JavascriptExecutor js = (JavascriptExecutor) driver;
			/*
			 * js.executeScript("arguments[0].scrollIntoView();",
			 * Report_PayrollComparison.filterSelectDivisionDropdown( driver), Division);
			 * 
			 * 
			 * wb.selectSingleValue(Report_PayrollComparison.filterSelectDivisionDropdown(
			...
			//JavascriptExecutor js = (JavascriptExecutor) driver;
			js.executeScript("arguments[0].scrollIntoView();",
					Report_PayrollComparison.filterSelectProjectFormOption(driver, Projects));
			Report_PayrollComparison.filterSelectProjectFormOption(driver, Projects).click();
			test.log(LogStatus.PASS, Projects + " Project Selected Successfully");

			// JavascriptExecutor js = (JavascriptExecutor) driver;
			// js.executeScript("window.scrollBy(0,150)", "");//Scroll down

			js.executeScript("arguments[0].scrollIntoView();", Report_PayrollComparison.selectFromDate(driver));
			wb.selectSingleValue(Report_PayrollComparison.selectFromDate(driver), "Apr - 2019");
			test.log(LogStatus.PASS, " From Date Selected Successfully");
			wb.selectSingleValue(Report_PayrollComparison.selectToDate(driver), "Oct - 2019");
			test.log(LogStatus.PASS, " To Date Selected Successfully");
			

			js.executeScript("arguments[0].click();", Report_PayrollComparison.btnBuildReport(driver));
		...
		try {
			System.out.println(Report_PayrollComparison.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_PayrollComparison.reportHeaderReportName(driver).getText(),
					"Payroll Comparison");
		...
		// filter testing
		Report_PayrollComparison.filterWindowButton(driver).click();
		Thread.sleep(3000);
		Report_PayrollComparison.filterSelectProjectFormOption(driver, ProjectNAME1).click();
...
//		testCount(contPkgSize);
		...
		Report_PayrollComparison.filterWindowButton(driver).click();
		Thread.sleep(3000);
		Report_PayrollComparison.filterSelectProjectFormOption(driver, ProjectNAME2).click();
...
//		testCount(contPkgSize);
		...
		//Raja s PDF Option is removed 
		
		/*
		 * // Check for pdf is generated or not take screenshot
		 * Report_PayrollComparison.pdfLink(driver).click(); test.log(LogStatus.PASS,
		 * "Clicked on Export To PDF Link "); wb.getWindowID3();
		 ...
		 * driver.switchTo().window(wb.map.get("childID")); test.log(LogStatus.PASS,
		 * "Clicked on Build Report Button ");
		 */

		// Excel file
		Report_PayrollComparison.excelLink(driver).click();
		...
		if (wb.isFileDownloaded("Payroll_Comparison.xls") == true) {
		...
		Thread.sleep(3000);
		Report_PayrollComparison.addFilterBtnOutside(driver).click();
		test.log(LogStatus.PASS, "Clicked on Add Filter Button From Filter");
		wb.getWindowID3();
		driver.switchTo().window(wb.map.get("subchildID"));
		driver.manage().window().maximize();
		wb.waitForElemnet(Report_PayrollComparison.selectAnFilterDropdown(driver));
		wb.selectSingleValueByIndex(Report_PayrollComparison.selectAnFilterDropdown(driver), 1);
		Thread.sleep(3000);
		test.log(LogStatus.PASS, " Filter Selected Successfully From Dropdown");
		wb.selectSingleValueByIndex(Report_PayrollComparison.selectAFilterCondition(driver), 1);
		Thread.sleep(3000);
		test.log(LogStatus.PASS, "Filter Condition Selected Successfully ");
		Report_PayrollComparison.addFilterParameter1(driver).click();
		Thread.sleep(1000);
		wb.selectSingleValueFromList(Report_PayrollComparison.selectParameter1(driver), "Enrolled");
		test.log(LogStatus.PASS, "Parameter Selected Successfully ");
		Report_PayrollComparison.addFilterBtnInside(driver).click();
	...
	public void testCount(int contPkgSize) {
		List<WebElement> countOfContractorPkg = Report_PayrollComparison.countContractorPkg(driver);
