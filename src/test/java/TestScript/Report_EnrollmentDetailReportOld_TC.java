...
public class Report_EnrollmentDetailReportOld_TC extends BaseClass {
	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "Enrollment Detail Report";

	@Test(dataProvider = "excelData")
	public void testEnrollmentDetailReportOld(String ProgramName, String Division, String Projects, String userName,
		...
		try {
			...
			Thread.sleep(5000);
			Report_EnrollmentListReportOld.selectReport(driver, selectReportName).click();
			test.log(LogStatus.PASS, selectReportName + " Report is Selected");
			wb.getWindowIDs();
			driver.switchTo().window(wb.map.get("childID"));
			driver.switchTo().frame(0);
			// wb.selectSingleValue(Report_EnrollmentDetailReportOld.selectDivision(driver),
			// Division);
			// test.log(LogStatus.PASS, " Divisions Selected Successfully");
			// Thread.sleep(3000);

			try {
				wb.selectSingleValue(Report_EnrollmentDetailReportOld.selectProjects(driver), Projects);
			...
			wb.selectSingleValue(Report_EnrollmentDetailReportOld.selectContractors(driver), "All Contractors");
		...
		try {
			...
			System.out.println(Report_EnrollmentDetailReportOld.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_EnrollmentDetailReportOld.reportHeaderReportName(driver).getText(),
					"Enrollment Detail");
