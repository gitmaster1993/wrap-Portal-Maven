...
public class Report_EnrollmentReportTC extends BaseClass {
	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "Enrollment Report  ";// 2 spaces in name
//	String project1 = "120 Piedmont Student Housing";
//	String project2 = "712 West Peachtree";

	
	@Test(dataProvider = "excelData")
	public void testEnrollmentReport(String ProgramName, String Division, String Projects,String userName, String password ,String ProjectNAME1,String ProjectNAME2) throws InterruptedException {
		try {
//			WPO_LoginActionClientWise.logInLockton(driver);
			...
			Thread.sleep(5000);
			Report_EnrollmentReport.selectReport(driver, selectReportName).click();
			test.log(LogStatus.PASS, selectReportName + " Report is Selected");
			wb.getWindowIDs();
			driver.switchTo().window(wb.map.get("childID"));
			
			//Raja - No option in Filters for Division
			
			/*
			 * wb.selectSingleValue(Report_EnrollmentReport.filterSelectDivisionDropdown(
			 * driver), Division); test.log(LogStatus.PASS, Division +
			 * " Selected Successfully");
			 */
			JavascriptExecutor js = (JavascriptExecutor)driver;
			js.executeScript("arguments[0].scrollIntoView();", Report_EnrollmentReport.filterSelectProjectFormOption(driver, Projects));
			Report_EnrollmentReport.filterSelectProjectFormOption(driver, Projects).click();
			test.log(LogStatus.PASS, Projects + " Project Selected Successfully");
            Thread.sleep(2000);
			Report_EnrollmentReport.btnBuildReport(driver).click();
		...
		try {
			System.out.println(Report_EnrollmentReport.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_EnrollmentReport.reportHeaderReportName(driver).getText(), "Enrollment Report");
...
//		testCount(contPkgSize);
		...
		if (wb.isFileDownloaded("Enrollment_Report.xls") == true) {
