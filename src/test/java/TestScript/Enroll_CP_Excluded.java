...
//			AddContractor_Pkg_Page.selectProgramSearchBox(driver).sendKeys("Sony Tower Test",Keys.TAB);
	...
	public void enrollContractor_Pkg_Excluded_Test() throws Throwable {
		...
		//wb.waitForElemnet(AddContractor_Pkg_SubContractorInfo.btnSaveAndContinue(driver));
		 ...
		 test.log(LogStatus.PASS,"Save and continue at subcontractor info page is clicked");
		Thread.sleep(5000);
		}catch (Exception e) {
			test.log(LogStatus.PASS,"Save and continue at subcontractor info page is clicked");
		}
		...
		try {
			...
			AddContractor_Pkg_Required_Documents_Page.btnSaveAndContinue(driver).click();
			test.log(LogStatus.PASS,"Save and continue at Req doc page is clicked");
		}catch (Exception e) {
			test.log(LogStatus.PASS,"Save and continue at Req doc page is not clicked");
		}

		// Osha EMR Page
		try {
			wb.moveToElement(AddContractor_Pkg_OSHA_EMR_Page.checkBoxA(driver));
			...
			test.log(LogStatus.PASS, "OSHA and EMR page is clicked");
		...
		//wb.mouseMoveToElement(AddContractor_Pkg_Enrollment_Summary_Page.btnEnrollmentSummaryPageConfirmAndContinue(driver));
			wb.scrollToEndt();
		js.executeScript("arguments[0].click();",
		...
		test.log(LogStatus.INFO, "Clicked On Enrollment Summary Confirm And Continue Button");
			...
			System.out.println("cannot click om confirm and continue");
		}
		...
		try {
			...
			test.log(LogStatus.INFO, "Clicked On  Continue Button with out sending an email");
