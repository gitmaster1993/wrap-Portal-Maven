...
import ObjectRepository.Report_OCIPMetric;
import ObjectRepository_NCP.Report_NCP_ContractorInsuranceCostTrackingReport;

public class Report_OCIPMetric_TC extends BaseClass {

	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "OCIP Metric";

	@Test(dataProvider = "excelData")
	public void testOCIPMetric(String ProgramName, String Division, String Projects, String userName, String password,
		...
		try {
			...
			Thread.sleep(5000);
			Report_OCIPMetric.selectReport(driver, selectReportName).click();
			test.log(LogStatus.PASS, selectReportName + " Report is Selected");
			wb.getWindowIDs();
			driver.switchTo().window(wb.map.get("childID"));
			wb.selectSingleValue(Report_OCIPMetric.filterSelectDivisionDropdown(driver), Division);
			test.log(LogStatus.PASS, " Divisions Selected Successfully");
			Report_OCIPMetric.filterSelectProjectFormOption(driver, Projects).click();
			test.log(LogStatus.PASS, " Project Selected Successfully");

			Report_OCIPMetric.btnBuildReport(driver).click();
		...
		try {
			System.out.println(Report_OCIPMetric.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_OCIPMetric.reportHeaderReportName(driver).getText(), "OCIP Metric");
