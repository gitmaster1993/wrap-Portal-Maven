...
public class Report_ContractorStructureTC extends BaseClass
{
	...
	//Raja
	String selectReportName="Contractor Structure";  //Contractor Structure By Program"; 
...
//		WPO_LoginActionClientWise.logInLockton(driver);
		 ...
		 * Report_ContractorStructure.filterSelectDivisionDropdown(driver));
		 * wb.selectSingleValue(Report_ContractorStructure.filterSelectDivisionDropdown(
		...
		/*
		 * js.executeScript("arguments[0].scrollIntoView();",
		 * Report_ContractorStructure.filterSelectProjectFormOption(driver,Projects));
		 * Report_ContractorStructure.filterSelectProjectFormOption(driver,
		 * Projects).click(); test.log(LogStatus.PASS,Projects+
		 * " Project Selected Successfully"); System.out.println(Projects+
		 * " Project Selected Successfully");
		 */
		
		js.executeScript("arguments[0].scrollIntoView();",Report_ContractorStructure.btnBuildReport(driver));
		Report_ContractorStructure.btnBuildReport(driver).click();
		test.log(LogStatus.INFO, " Clicked On Build Report Button");
		System.out.println("Clicked On Build Report Button");
		Thread.sleep(3000);
	...
	try {
		...
		Assert.assertEquals(Report_ContractorStructure.reportHeaderReportName(driver).getText(), "Contractor Structure");
	...
	//filter testing
			...
			System.out.println("filter window button is clicked");
			Thread.sleep(3000);
			...
			System.out.println("First project Selected succesfully");
			...
			System.out.println("Second  project Selected succesfully");
			ReportModule.btnBuildReport(driver).click();
			test.log(LogStatus.PASS, "Clicked on Build Report Button ");
			System.out.println("Build report button at project 2");
			contPkgSize=11;
			//testCount(contPkgSize);
		...
		//Check for pdf is generated or not take screenshot
			...
			System.out.println("Export to PDF link");
			wb.getWindowID3();
			...
			System.out.println("Export to Excel link");
			Thread.sleep(5000);
			//isFileDownloaded is the std method which accept the name of file and confirms the download
			//Assert.assertTrue(wb.isFileDownloaded("Claims_Coding_-_Contractor_Summary_Report.xls"), "Failed to download Expected document");
			...
			System.out.println("Filter window after excel downlaoded");
			Thread.sleep(3000);
