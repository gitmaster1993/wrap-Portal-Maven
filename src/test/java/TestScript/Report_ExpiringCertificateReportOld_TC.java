...
import ObjectRepository.Report_ExpiringCertificateReportOld;

public class Report_ExpiringCertificateReportOld_TC extends BaseClass {
	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "Expiring Certificate Report";

	@Test(dataProvider = "excelData")
	public void testExpiringCertificateReportOld(String ProgramName, String Division, String Projects, String userName,
		...
		try {
			...
			Thread.sleep(5000);
			Report_ExpiringCertificateReportOld.selectReport(driver, selectReportName).click();
			test.log(LogStatus.PASS, selectReportName + " Report is Selected");
			wb.getWindowIDs();
			driver.switchTo().window(wb.map.get("childID"));
			driver.switchTo().frame(0);
			// wb.selectSingleValue(Report_ExpiringCertificateReportOld.selectDivision(driver),
			// Division);
			// test.log(LogStatus.PASS, " Divisions Selected Successfully");
			Thread.sleep(3000);
			wb.selectSingleValue(Report_ExpiringCertificateReportOld.selectProjects(driver), Projects);
			test.log(LogStatus.PASS, " Project Selected Successfully");
			Thread.sleep(3000);
			wb.selectSingleValue(Report_ExpiringCertificateReportOld.selectContractors(driver), "All Contractors");
		...
		try {
			...
			System.out.println(Report_ExpiringCertificateReportOld.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_ExpiringCertificateReportOld.reportHeaderReportName(driver).getText(),
