...
import ObjectRepository.Report_ClaimsSummaryReportOld;
import ObjectRepository.Report_EnrollmentListReportOld;

public class Report_ClaimsSummaryReportOld_TC extends BaseClass {

	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "Claims Summary Report";

	@Test(dataProvider = "excelData")
	public void testClaimsSummaryReportOld(String ProgramName, String Division, String Projects, String userName,
		...
		try {
			...
			Thread.sleep(5000);
			Report_ClaimsSummaryReportOld.selectReport(driver, selectReportName).click();
			test.log(LogStatus.PASS, selectReportName + " Report is Selected");
			wb.getWindowIDs();
			driver.switchTo().window(wb.map.get("childID"));
			driver.switchTo().frame(0);
			// wb.selectSingleValue(Report_ClaimsSummaryReportOld.selectDivision(driver),
			// Division);
			// test.log(LogStatus.PASS, " Divisions Selected Successfully");
			// Thread.sleep(3000);

			try {
				wb.selectSingleValue(Report_ClaimsSummaryReportOld.selectProjects(driver), Projects);
			...
			Report_ClaimsSummaryReportOld.btnBuildReport(driver).click();
		...
		try {
			...
			System.out.println(Report_ClaimsSummaryReportOld.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_ClaimsSummaryReportOld.reportHeaderReportName(driver).getText(),
