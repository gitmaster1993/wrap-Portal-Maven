...
import ObjectRepository.Report_EnrollmentReport;
import ObjectRepository.Report_EstimatedEndDates;

public class Report_EstimatedEndDatesTC extends BaseClass {

	WebDriverCommonLib wb = new WebDriverCommonLib();
	String selectReportName = "Estimated End Dates";
...
//	String project2 = "712 West Peachtree";
	...
	public void testEstimatedEndDates(String ProgramName, String Division, String Projects,String userName, String password ,String ProjectNAME1,String ProjectNAME2)
...
//			WPO_LoginActionClientWise.logInLockton(driver);
			...
			Thread.sleep(2000);
			wb.selectSingleValue(Report_EstimatedEndDates.selectProgramDropdown(driver), ProgramName);
			test.log(LogStatus.INFO, ProgramName + " Program Select Form Dropdown");
			Thread.sleep(5000);
			Report_EstimatedEndDates.selectReport(driver, selectReportName).click();
			test.log(LogStatus.PASS, selectReportName + " Report is Selected");
			wb.getWindowIDs();
			driver.switchTo().window(wb.map.get("childID"));
			
			/*
			 * wb.selectSingleValue(Report_EstimatedEndDates.filterSelectDivisionDropdown(
			 * driver), Division); test.log(LogStatus.PASS, Division +
			 * " Selected Successfully");
			 * Report_EstimatedEndDates.filterSelectProjectFormOption(driver,
			 * Projects).click(); test.log(LogStatus.PASS, Projects +
			 * " Project Selected Successfully");
			 */
			JavascriptExecutor js = (JavascriptExecutor)driver;
			js.executeScript("arguments[0].scrollIntoView();", Report_EstimatedEndDates.btnBuildReport(driver));

			Report_EstimatedEndDates.btnBuildReport(driver).click();
		...
		try {
			System.out.println(Report_EstimatedEndDates.reportHeaderReportName(driver).getText());
			Assert.assertEquals(Report_EstimatedEndDates.reportHeaderReportName(driver).getText(),
					"Estimated End Dates");
		...
		// filter testing
		Report_EnrollmentReport.filterWindowButton(driver).click();
		Thread.sleep(3000);
		Report_EnrollmentReport.filterSelectProjectFormOption(driver, ProjectNAME1).click();
...
//		testCount(contPkgSize);
		...
		Report_EnrollmentReport.filterWindowButton(driver).click();
		Thread.sleep(3000);
		Report_EnrollmentReport.filterSelectProjectFormOption(driver, ProjectNAME2).click();
...
//		testCount(contPkgSize);
		...
		Report_EnrollmentReport.pdfLink(driver).click();
		...
		// Excel file
		Report_EnrollmentReport.excelLink(driver).click();
		...
		if (wb.isFileDownloaded("Estimated_End_Dates.xls") == true) {
		...
		Thread.sleep(3000);
		Report_EnrollmentReport.addFilterBtnOutside(driver).click();
		test.log(LogStatus.PASS, "Clicked on Add Filter Button From Filter");
		wb.getWindowID3();
		driver.switchTo().window(wb.map.get("subchildID"));
		driver.manage().window().maximize();
		wb.waitForElemnet(Report_EnrollmentReport.selectAnFilterDropdown(driver));
		wb.selectSingleValueByIndex(Report_EnrollmentReport.selectAnFilterDropdown(driver), 1);
		Thread.sleep(3000);
		test.log(LogStatus.PASS, " Filter Selected Successfully From Dropdown");
		wb.selectSingleValueByIndex(Report_EnrollmentReport.selectAFilterCondition(driver), 1);
		Thread.sleep(3000);
		test.log(LogStatus.PASS, "Filter Condition Selected Successfully ");
		Report_EnrollmentReport.addFilterParameter1(driver).click();
		Thread.sleep(1000);
		wb.selectSingleValueFromList(Report_EnrollmentReport.selectParameter1(driver), "Enrolled");
		test.log(LogStatus.PASS, "Parameter Selected Successfully ");
		Report_EnrollmentReport.addFilterBtnInside(driver).click();
	...
	public void testCount(int contPkgSize) {
		...
		List<WebElement> countOfContractorPkg = Report_EnrollmentReport.countContractorPkg(driver);
